export default function DataEditor({ reportData, setReportData }) {
  ...
  const parseCSV = (text) => {
    ...
    return dataRows.map(row => {
      ...
      return {
        ...
        source: cols[cols.length - 1]?.replace(/^"|"$/g, "").trim() || "",
        align: "left"
      };
    });
  };
  ...
  const handleCSV = (sectionKey, text) => {
    setReportData({
      ...reportData,
      [sectionKey]: parseCSV(text)
    });
  };

  return (
    <div className="bg-white p-4 rounded-xl shadow space-y-4">
      <h2 className="text-lg font-bold text-gray-700">Report Editor</h2>
      ...
      {[
        ...
        <div key={sec.key}>
          <label className="block font-semibold mb-1">{sec.label} CSV</label>
          <textarea
